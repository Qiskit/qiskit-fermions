...
class TestExcitationSpanOptimizationModel:
    def test_empty_excitations_builds_valid_model_for_avg(self):
        ...
        assert set(model.P2.data()) == set()
        assert set(model.P4.data()) == set()

        # `avg` objective should not define `max_obj`
        assert not hasattr(model, "max_obj")
        assert isinstance(model.obj, Objective)

    def test_ignores_terms_reducing_to_length_zero_or_one(self):
    ...
    def test_repeated_indices_cancel_to_a_pair(self):
    ...
    def test_processed_pairs_and_quads_are_partitioned_correctly(self):
        ...
        assert set(model.P2.data()) == {(0, 2)}
        assert set(model.P4.data()) == {(0, 1, 2, 3)}

    def test_duplicate_processed_pairs_are_deduplicated(self):
    ...
    def test_length_three_after_processing_raises_assertion(self):
    ...
    def test_named_constraints_exist_for_pair_when_max_objective_is_used(self):
