...
static int test_get_support(void) {
    ...
    QfTransferVertexOperator *empty = qf_transfer_op_zero();
    passed_all = passed_all && (qf_transfer_op_num_support(empty) == 0);

    qf_transfer_op_free(op);
    qf_transfer_op_free(empty);
...
static int test_scaled_add(void) {
    ...
    QfTransferVertexOperator *op_a = qf_transfer_op_new(1, 2, &coeff, left_a, right_a, boundaries);
    uint32_t left_b[2] = {2, 3};
    uint32_t right_b[2] = {3, 4};
    QfTransferVertexOperator *op_b = qf_transfer_op_new(1, 2, &coeff, left_b, right_b, boundaries);
    ...
    QfTransferVertexOperator *difference = qf_transfer_op_scaled_add(op_a, op_b, &minus_one);
    QfTransferVertexOperator *restored =
        qf_transfer_op_scaled_add(difference, op_b, &plus_one_early);
    QfTransferVertexOperator *simplified_restored = qf_transfer_op_simplify(restored, 1e-10);
    QfTransferVertexOperator *simplified_a = qf_transfer_op_simplify(op_a, 1e-10);
    passed_all = passed_all && qf_transfer_op_equiv(simplified_restored, simplified_a, 1e-10);
    qf_transfer_op_free(simplified_restored);
    qf_transfer_op_free(simplified_a);

    // A factor of +1 is plain addition.
    QkComplex64 plus_one = {1.0, 0.0};
    QfTransferVertexOperator *summed = qf_transfer_op_scaled_add(op_a, op_b, &plus_one);
    QfTransferVertexOperator *added = qf_transfer_op_add(op_a, op_b);
    passed_all = passed_all && qf_transfer_op_equal(summed, added);

    // Scaling by zero still appends the terms; only their coefficients vanish.
    QkComplex64 zero_factor = {0.0, 0.0};
    QfTransferVertexOperator *scaled_zero = qf_transfer_op_scaled_add(op_a, op_b, &zero_factor);
    passed_all = passed_all && (qf_transfer_op_len(scaled_zero) == qf_transfer_op_len(added));
    QfTransferVertexOperator *chopped = qf_transfer_op_simplify(scaled_zero, 1e-10);
    passed_all = passed_all && (qf_transfer_op_len(chopped) == qf_transfer_op_len(op_a));

    // Appending terms drops the grouping, exactly as `add` does.
    uint32_t groups_in[1] = {0};
    qf_transfer_op_set_groups(op_a, groups_in, 1);
    QfTransferVertexOperator *from_grouped = qf_transfer_op_scaled_add(op_a, op_b, &plus_one);
    passed_all = passed_all && !qf_transfer_op_has_groups(from_grouped);
    qf_transfer_op_del_groups(op_a);

    qf_transfer_op_free(op_a);
    qf_transfer_op_free(op_b);
    qf_transfer_op_free(difference);
    qf_transfer_op_free(restored);
    qf_transfer_op_free(summed);
    qf_transfer_op_free(added);
    qf_transfer_op_free(scaled_zero);
    qf_transfer_op_free(chopped);
    qf_transfer_op_free(from_grouped);
...
static int test_inplace_arithmetic(void) {
    ...
    uint32_t right_b[2] = {3, 4};
    QfTransferVertexOperator *op_b = qf_transfer_op_new(1, 2, &coeff, left_b, right_b, boundaries);
    QfTransferVertexOperator *scratch =
        qf_transfer_op_new(1, 2, &coeff, left_a, right_a, boundaries);

    bool passed_all = true;

    // add_inplace == add
    QfTransferVertexOperator *expected_sum = qf_transfer_op_add(op_a, op_b);
    qf_transfer_op_add_inplace(scratch, op_b);
    passed_all = passed_all && qf_transfer_op_equal(scratch, expected_sum);

    // scaled_add_inplace == scaled_add
    QkComplex64 factor = {-2.0, 0.5};
    QfTransferVertexOperator *expected_scaled = qf_transfer_op_scaled_add(op_a, op_b, &factor);
    QfTransferVertexOperator *scaled =
        qf_transfer_op_new(1, 2, &coeff, left_a, right_a, boundaries);
    qf_transfer_op_scaled_add_inplace(scaled, op_b, &factor);
    passed_all = passed_all && qf_transfer_op_equal(scaled, expected_scaled);

    // mul_inplace == mul
    QkComplex64 scalar = {0.0, 3.0};
    QfTransferVertexOperator *expected_scalar = qf_transfer_op_mul(op_a, &scalar);
    QfTransferVertexOperator *multiplied =
        qf_transfer_op_new(1, 2, &coeff, left_a, right_a, boundaries);
    qf_transfer_op_mul_inplace(multiplied, &scalar);
    passed_all = passed_all && qf_transfer_op_equal(multiplied, expected_scalar);

    // The two appending operations drop the grouping, ...
    QfTransferVertexOperator *grouped_add =
        qf_transfer_op_new(1, 2, &coeff, left_a, right_a, boundaries);
    uint32_t groups_in[1] = {0};
    qf_transfer_op_set_groups(grouped_add, groups_in, 1);
    qf_transfer_op_add_inplace(grouped_add, op_b);
    passed_all = passed_all && !qf_transfer_op_has_groups(grouped_add);

    QfTransferVertexOperator *grouped_scaled =
        qf_transfer_op_new(1, 2, &coeff, left_a, right_a, boundaries);
    qf_transfer_op_set_groups(grouped_scaled, groups_in, 1);
    qf_transfer_op_scaled_add_inplace(grouped_scaled, op_b, &factor);
    passed_all = passed_all && !qf_transfer_op_has_groups(grouped_scaled);

    // ... while scaling the coefficients keeps it.
    QfTransferVertexOperator *grouped_mul =
        qf_transfer_op_new(1, 2, &coeff, left_a, right_a, boundaries);
    qf_transfer_op_set_groups(grouped_mul, groups_in, 1);
    qf_transfer_op_mul_inplace(grouped_mul, &scalar);
    passed_all = passed_all && qf_transfer_op_has_groups(grouped_mul);

    qf_transfer_op_free(op_a);
    qf_transfer_op_free(op_b);
    qf_transfer_op_free(scratch);
    qf_transfer_op_free(expected_sum);
    qf_transfer_op_free(expected_scaled);
    qf_transfer_op_free(scaled);
    qf_transfer_op_free(expected_scalar);
    qf_transfer_op_free(multiplied);
    qf_transfer_op_free(grouped_add);
    qf_transfer_op_free(grouped_scaled);
    qf_transfer_op_free(grouped_mul);
