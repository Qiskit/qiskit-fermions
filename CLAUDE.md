...
`make testpython` is what actually *runs* the Python doctests (the docstring/`.pyi` doctests under `python/qiskit_fermions/`, collected via Sybil during a normal `pytest` run), but it does **not** build anything itself — it is a bare `pytest` invocation with no prerequisites. The stubs carry the API docstrings *and doctests*, so they must be regenerated for those doctests to reflect current source: run **`make pyext-dev` first** (it depends on `pystubs-dev`, so it recompiles `pyext` *and* regenerates the `.pyi` stubs). Skipping it silently tests a stale `_lib*.so` against stale stubs. `make testc`, by contrast, *does* rebuild what it needs: it depends on `build-clib-dev cheader`, recompiling `cext` and regenerating the C header via `cbindgen`.
