...
impl TermSortKey for TransferVertexOperatorTermView<'_> {
...
impl GroupedTerm for TransferVertexOperatorGroupTermView<'_> {
    fn group(&self) -> u32 {
        self.group
    }
}
                            ...
                            .map(|&idx| {
                                ...
                                if let Some(range) = group_term_range(groups, idx) {
                                    for i in range {
                                        ...
                                        acc._append_term(
                                            self.coeffs[i],
                                            &self.left_indices[start..end],
                                            &self.right_indices[start..end],
                                        );
                                    }
                                }
                                acc
...
mod tests {
    ...
    // The Eq. (7) relations are verified a second time, independently, in `tests/python/operators/test_transfer_vertex_operator.py`
    ...
    fn test_iter_from_terms_round_trip() {
        ...
        let round_trip = TransferVertexOperator::from_terms(op.iter());
