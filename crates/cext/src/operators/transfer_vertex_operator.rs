...
/// many would make :c:func:`qf_transfer_op_num_groups` report groups that no term carries.
///
/// .. seealso::
///    The explanation on :ref:`grouping_explanation`.
///
...
///
/// .. note::
///    Like :c:func:`qf_transfer_op_add`, this appends the terms of ``right`` without combining them with
///    those of ``left``, so the result tracks no group indices. Call :c:func:`qf_transfer_op_simplify` to
...
///
///     QfTransferVertexOperator *one = qf_transfer_op_one();
///     QfTransferVertexOperator *other = qf_transfer_op_one();
///
///     // Subtracting via a factor of -1 leaves the two terms cancelling each other out.
///     QkComplex64 factor = {-1.0, 0.0};
///     QfTransferVertexOperator *result = qf_transfer_op_scaled_add(one, other, &factor);
///
///     QfTransferVertexOperator *simplified = qf_transfer_op_simplify(result, 1e-10);
///     assert(qf_transfer_op_len(simplified) == 0);
///
/// @endrst
#[unsafe(no_mangle)]
pub unsafe extern "C" fn qf_transfer_op_scaled_add(
    left: *const TransferVertexOperator,
    right: *const TransferVertexOperator,
...
/// @rst
///
/// The in-place counterpart of :c:func:`qf_transfer_op_add`, which saves copying ``left`` into a freshly
/// allocated result.
///
/// .. caution::
///    This function resets the operator's ``groups`` attribute to ``NULL``.
...
///     :linenos:
///
///     QfTransferVertexOperator *left = qf_transfer_op_one();
///     QfTransferVertexOperator *right = qf_transfer_op_one();
///
///     qf_transfer_op_add_inplace(left, right);
///
///     assert(qf_transfer_op_len(left) == 2);
///
/// @endrst
#[unsafe(no_mangle)]
pub unsafe extern "C" fn qf_transfer_op_add_inplace(
    left: *mut TransferVertexOperator,
    right: *const TransferVertexOperator,
) {
    // SAFETY: Per documentation, the pointers are non-null and aligned.
...
/// @rst
///
/// The in-place counterpart of :c:func:`qf_transfer_op_scaled_add`, computing ``left + factor * right``
...
///     QkComplex64 factor = {-1.0, 0.0};
///     qf_transfer_op_scaled_add_inplace(left, right, &factor);
///
///     QfTransferVertexOperator *simplified = qf_transfer_op_simplify(left, 1e-10);
///     assert(qf_transfer_op_len(simplified) == 0);
///
/// @endrst
#[unsafe(no_mangle)]
pub unsafe extern "C" fn qf_transfer_op_scaled_add_inplace(
    left: *mut TransferVertexOperator,
    right: *const TransferVertexOperator,
    factor: *const Complex64,
) {
...
/// @rst
///
/// The in-place counterpart of :c:func:`qf_transfer_op_mul`, which saves copying the operator into a
...
///
///     qf_transfer_op_mul_inplace(op, &scalar);
///
/// @endrst
#[unsafe(no_mangle)]
pub unsafe extern "C" fn qf_transfer_op_mul_inplace(
    op: *mut TransferVertexOperator,
